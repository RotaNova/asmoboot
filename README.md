...
- 1.请先修改.env文件下的环境参数
  ...
  *MINIO_ENDPOINT 为部署服务器可访问IP+9000端口 不能使用localhost以及127.0.0.1 例如 http://192.168.0.1:9000
...
- 2.将该目录下所有文件及文件夹复制到服务器上干净目录，使用(docker-compose up -d)   一键部署
...
- 1.成功启动后/db/data目录为mysql数据库目录，请注意备份
- 2./minio  data下文件为minio桶目录
